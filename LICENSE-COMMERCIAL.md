# Newt-SFU Commercial License Notice

Copyright (c) 2026 NewtSpeak and contributors

## 商业使用说明（简体中文）

本软件的免费许可**仅适用于非商业 / 个人用途**（见 `LICENSE` 与 `LICENSE-NONCOMMERCIAL.md`）。

### 不允许免费商用

在未获得书面商业许可前，您**不得**将本软件或其衍生作品用于任何商业目的，包括但不限于：

- 公司、个体工商户或其他营利组织的生产 / 运营环境
- 对外提供收费服务、增值服务，或以本软件吸引客户 / 流量
- 将本软件嵌入商业产品、设备、解决方案或交付物
- 以 SaaS、托管、云服务、代运维等形式向第三方提供
- 转售、OEM、白标、二次分发用于商业目的
- 商业培训、商业咨询项目中的交付与部署

### 如何获取商业许可

请通过以下方式联系版权方洽谈商业授权（定价、范围、期限、是否闭源等可协商）：

- GitHub：https://github.com/NewtSpeak/Newt-SFU/issues  
  （请创建 Issue，标题建议以 `[Commercial License]` 开头）
- 或通过仓库所有者 / 组织 NewtSpeak 的公开联系渠道

商业许可通常可包含（以最终书面合同为准）：

- 闭源使用与分发权利
- 免除非商业许可中的强制开源义务
- 多实例 / 多产品部署范围
- 技术支持与升级约定（如有）

### 重要声明

- 本文件本身**不是**自动生效的商业合同；商业权利仅在双方签署书面商业许可后授予。
- 违反非商业许可或在无商业许可情况下进行商业使用，将导致免费许可终止，并可能承担法律责任。

---

## English Summary

Commercial use of this software is **not free**.

The free grant is limited to noncommercial/personal use under `LICENSE` and
`LICENSE-NONCOMMERCIAL.md`. Any commercial use requires a separate paid
commercial license obtained in writing from the copyright holder.

Contact: open a GitHub Issue at  
https://github.com/NewtSpeak/Newt-SFU/issues  
with a title starting with `[Commercial License]`, or contact the NewtSpeak
organization maintainers through their public channels.

This file is a notice only; commercial rights are granted solely by a signed
commercial license agreement.
